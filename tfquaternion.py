...
def quaternion_multiply(a, b):
    ...
    return Quaternion(tf.stack((w, x, y, z), axis=-1))
...
class Quaternion(object):
    ...
    def __init__(self, wxyz=(1, 0, 0, 0), dtype=tf.float32, name=None):
        ...
        self.filter_vals(self._q)

    def filter_vals(self, q):
        zs = tf.ones_like(q) * 1e-4
        self._q = tf.where(tf.is_nan(q), zs, q)
    ...
    def norm(self, keepdims=True):
        ...
        n = tf.reduce_sum(tf.square(self._q), axis=-1, keepdims=keepdims)
        #n = tf.where(n < 1e-6, tf.ones_like(n) * 1e-4, n)
        return n

    @scope_wrapper
    def abs(self, keepdims=True):
        """Return the square root of the norm of the quaternion."""
        sr = safe_norm(self._q)
        #sr = tf.where(sr < 1e-6, tf.ones_like(sr) * 1e-4, sr)
        return sr
