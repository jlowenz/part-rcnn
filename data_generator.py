...
from shared_batch import SharedBatch
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None, use_mini_mask=False):
    ...
    with Timed("augmentation"):
        if augmentation:
            ...
            if cfg.enable_segmentation_extension:
                gt_seg = det.augment_image(gt_seg)
            ...
            if cfg.enable_segmentation_extension:
                assert gt_seg.shape == gt_seg_shape
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        error_count = 0
        try:
            image_index = (idx*self.batch_size) % self.N # ensure we can't go oob
            b = 0
            while True:
                ...
                if b == 0:
                    ...
                    if cfg.enable_segmentation_extension:
                        batch_gt_seg = np.zeros((self.batch_size,1,) + gt_seg.shape, dtype=np.float32)
                ...
                if b >= self.batch_size:
                    ...
                    if cfg.enable_segmentation_extension:
                        inputs.append(batch_gt_seg)

                    return inputs, outputs
                else:
                    image_index = (image_index + 1) % self.N
...
class LoadSet(object):
    def __init__(self):
        ...
        self.rpn_rois = None
        self.rois = None
        self.mrcnn_class_ids = None
        self.mrcnn_bbox = None
        self.mrcnn_mask = None
            
class DataGeneratorMP(keras.utils.Sequence):
    def __init__(self, dataset, config, augmentation=None,
        ...
        self.N = len(self.image_ids)
        # shuffle the ids
        self.id_order = npr.permutation(self.N)
        self.queue_size = queue_size

        # create prototypes
        ls = self.load(0)
        
        # create the shared batch objects
        print("Creating batches...")
        self.batches_ = [SharedBatch(i, self.N,
                                     self.queue_size,
                                     ls,
                                     self.config,
                                     self.anchors,
                                     self.random_rois,
                                     self.detection_targets) for i in range(queue_size)]
        
        
        # launch the load threads
        print("Creating background threads")
        self.procs_ = [mp.Process(target=self.load_thread, args=(b,)) for b
                        in self.batches_]
    ...
    def load(self, image_index):
        print("Loading {}".format(image_index))
        cfg = self.config.PN_CONFIG
        image_id = self.image_ids[image_index]
        ls = LoadSet()
        ...
        ls.rpn_match, ls.rpn_bbox = build_rpn_targets(ls.image.shape, self.anchors,
                                                    ...
                                                    self.config)
        if self.random_rois:
            ls.rpn_rois = generate_random_rois(
                ls.image.shape, self.random_rois, ls.gt_class_ids, ls.gt_boxes)
            if self.detection_targets:
                ls.rois, ls.mrcnn_class_ids, ls.mrcnn_bbox, ls.mrcnn_mask =\
                  build_detection_targets(
                      ls.rpn_rois, ls.gt_class_ids, ls.gt_boxes, ls.gt_masks, self.config)

        if ls.gt_boxes.shape[0] > self.config.MAX_GT_INSTANCES:
            ids = np.random.choice(
                np.arange(gt_boxes.shape[0]), self.config.MAX_GT_INSTANCES,
                replace=False)
            ls.gt_class_ids = ls.gt_class_ids[ids]
            ls.gt_boxes = ls.gt_boxes[ids]
            ls.gt_masks = ls.gt_masks[:,:,ids]
        return ls
        
    def load_thread(self, b):
        cfg = self.config.PN_CONFIG
        while True:
            print("load thread {} checking for write".format(b.index))
            with b.read_complete:
                print("Ready for read: {}".format(b.can_read()))
            ...
            try:
                ...
                while not batch_done:
                    ls = self.load(image_index)
                    # add to the batch
                    b.batch_image_meta[bi] = ls.image_meta
                    b.batch_rpn_match[bi] = ls.rpn_match[:, np.newaxis]
                    b.batch_rpn_bbox[bi] = ls.rpn_bbox
                    b.batch_images[bi] = mold_image(ls.image.astype(np.float32),
                                                    self.config)
                    if self.config.USE_DEPTH:
                        b.batch_depth[bi] = ls.depth
                    if self.config.USE_NORMALS:
                    ...
                    if self.random_rois:
                        b.batch_rpn_rois[bi] = ls.rpn_rois
                        if self.detection_targets:
                            b.batch_rois[bi] = ls.rois
                            b.batch_mrcnn_class_ids[bi] = ls.mrcnn_class_ids
                            b.batch_mrcnn_bbox[bi] = ls.mrcnn_bbox
                            b.batch_mrcnn_mask[bi] = ls.mrcnn_mask
                    bi += 1
                    # batch full?
                    if bi >= self.batch_size:
                        batch_done = True
                        b.enable_read()
                    else:
                        idx += 1
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print("getting item from {}".format(self.bindex))
            with batch.read_complete:
                print("can read: {}".format(batch.can_read()))
                batch.read_complete.wait_for(batch.can_read)
            print("proceeding to getitem {}".format(self.bindex))

            inputs = [batch.batch_images, batch.batch_image_meta,
                      batch.batch_rpn_match, batch.batch_rpn_bbox,
                      batch.batch_gt_class_ids, batch.batch_gt_boxes,
                      batch.batch_gt_masks]
            outputs = []

            if self.random_rois:
                inputs.extend([batch.batch_rpn_rois])
                if self.detection_targets:
                    inputs.extend([batch.batch_rois])
                    # Keras requires that output and targets have the same number of dimensions
                    batch_mrcnn_class_ids = np.expand_dims(
                        batch.batch_mrcnn_class_ids, -1)
                    outputs.extend(
                        [batch_mrcnn_class_ids, batch.batch_mrcnn_bbox,
                         batch.batch_mrcnn_mask])

            if self.config.USE_DEPTH:
                inputs.append(batch.batch_depth)
            if self.config.USE_NORMALS:
